...
@Override
    ...
    @Override
    public Commentaire UpdateCommentaire(Long commentaireId, String contenu, Long tacheId, Long userId) {
        // Recherche du commentaire par son ID
        Commentaire commentaire = iCommentaireRepository.findById(commentaireId)
                .orElseThrow(() -> new NotFoundException("Commentaire non trouvé avec l'ID : " + commentaireId));
        ...
        if (estEtudiantOuEncadrantDeTache(tache, userId)) {
            ...
            // Enregistrement du commentaire mis à jour
            return iCommentaireRepository.save(commentaire);
        } else {
            throw new NotFoundException("L'utilisateur avec l'ID : " + userId + " n'est pas autorisé à commenter la tâche avec l'ID : " + tacheId);
        }
