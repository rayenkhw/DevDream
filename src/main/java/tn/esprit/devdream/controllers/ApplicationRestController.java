...
public class ApplicationRestController {
    ...
    public Depot accepterApplication(@PathVariable("application-id") Long applicationId) {
        try {
            System.out.println("accepter applicationId: " + applicationId);



            Depot depot = applicationService.applicationaccept(applicationId);
            return depot;
        } catch (Exception e) {
  ...
  //  private static final String uploadDir = "C:\\Users\\saidi\\Desktop\\integration\\material-dashboard-angular2-master\\src\\assets";

    private static final String uploadDir = "C:\\Users\\user\\Desktop\\PI\\integration\\integration\\material-dashboard-angular2-master\\src\\assets";
    ...
    public void applicationrefuser(@PathVariable("application-id") Long applicationId) {
        try {
            System.out.println("refuser applicationId: " + applicationId);

            applicationService.applicationrefuser(applicationId);

        } catch (Exception e) {
            System.err.println("Error refusing application: " + e.getMessage());
            e.printStackTrace();
            // Handle the error appropriately
            // For example: return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
